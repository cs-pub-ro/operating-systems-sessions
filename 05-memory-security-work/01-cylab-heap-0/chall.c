/*
 * chall.c -- heap0: "my data isn't on the stack, so it must be safe".
 *
 * Two adjacent heap buffers are allocated: input_data, which the player
 * may write to directly through the menu, and safe_var, which the
 * challenge author believes cannot be reached from write_buffer().
 *
 * That belief relies on nothing overflowing input_data's allocation. The
 * write path uses scanf("%s", ...) with no length limit, so anything the
 * player types past INPUT_DATA_SIZE bytes keeps going -- straight into
...
/*
 * The win condition is inverted on purpose: safe_var starts out equal to
 * "bico", and the player wins by making it anything else. Nothing in this
 * program ever assigns a new value to safe_var -- so the only way to
 * change it is to corrupt it from the outside.
 */
