...
The site is generated by `build.py` and deployed by the `.github/workflows/pages.yml` workflow on every push to `master`.
...
The generator discovers everything at build time, by walking the directory tree.
...
* Directories named `solutions` are skipped, together with `.git`, `.github` and `site`.
  The list is the `EXCLUDED_DIRS` set in `build.py`.
* The title shown next to a task is the first level-one heading of its `README.md`.
  If the file has no heading, the directory name is used instead.
...
Install the only dependency, then build the site:
