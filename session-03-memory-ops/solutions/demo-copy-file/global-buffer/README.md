...
The buffer is allocated at program start (in the BSS / data segment) and reused for every read/write iteration.
`memcpy()` is used to move data from the read buffer into a local staging area before writing, demonstrating in-memory copy operations.
