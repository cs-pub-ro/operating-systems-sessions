...
Solution script and a containerized solver environment for the "heap3" challenge.
...
Runs the exploit against the challenge service started via `../deploy/README.md` (listening on `localhost:31013`).
...
docker run --rm -it --network host \
    -v "$(pwd):/solve" \
    heap3-solver \
    python3 /solve/exploit.py REMOTE HOST=127.0.0.1 PORT=31013
```
...
docker run --rm -it \
    -v "$(pwd):/solve" \
    heap3-solver \
    python3 /solve/exploit.py REMOTE HOST=1.2.3.4 PORT=31013
```
