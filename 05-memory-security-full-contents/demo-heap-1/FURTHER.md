...
1. Find the smallest winning payload.
   For demo-heap-1, "root" is only there to be non-empty (heap-0 style) or to match exactly (heap-1 style); how few bytes can you get away with?
1. Re-run against a local deployment and diff the exploit invocation against the local-binary one.

## Questions to answer

* **Why does writing past 5 bytes reach `access_level`?**
...
* **This is the template.** 02-cylab-heap-1 is the same moves; students should breeze through it after the demo.
