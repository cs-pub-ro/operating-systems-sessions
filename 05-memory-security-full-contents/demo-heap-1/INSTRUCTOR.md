# Instructor Notes: demo-heap-1

## Purpose

A warm-up, solved together at the board.
It is a near-clone of `02-cylab-heap-1`; the point is to walk the whole workflow once so the matching core challenge is a quick win and the room starts with momentum.
...
1. Build the payload in pwntools: `b"A" * 32 + "root"`.
...
* `access_level` starts as "user" and nothing in the code ever changes it — the only way it moves is the overflow.
* `scanf("%s")` is the entire bug.

## Verified values

* Overflow distance: 32 bytes. Winning payload: `b"A" * 32 + "root"`.
* Deploy port: 31001.
