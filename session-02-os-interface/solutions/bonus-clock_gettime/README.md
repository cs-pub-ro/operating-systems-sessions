...
The x86-64 syscall number for `clock_gettime` is **228**.
...
`clock_gettime` takes two arguments: the clock ID (an integer) and a pointer to a `struct timespec`.
We pass them as the first two arguments to `my_syscall()`, casting the pointer to `long` as the calling convention requires:
...
`time()` only needs whole seconds.
We read `CLOCK_REALTIME` via `my_clock_gettime()` and extract `tv_sec`.
If the caller passed a non-NULL `tloc`, we store the result there before returning it:
...
This mirrors exactly how glibc implements `time()` internally — it is just a thin wrapper around `clock_gettime(CLOCK_REALTIME, ...)`.
