...
This is a **fully solved demo**.
Read the code together with this README to understand how a program talks to the kernel *directly*, without going through the C standard library.

## What is a system call?

Your program runs in **user mode** and cannot touch hardware, files or other processes on its own.
...
In this session we build those wrappers **ourselves**.
...
The generic wrapper.
On **x86-64 Linux** a system call works like this:
...
We load these registers using inline assembly and execute the `syscall` instruction.
...
`my_write()` just forwards `fd`, `buf` and `count` to `my_syscall()`.

### Layer 3 — `my_puts()` (in `main.c`)

A convenience function like `puts`: it measures the string length by hand and calls `my_write()` to print the text plus a newline to `STDOUT_FILENO` (fd 1).
...
You should see `write(1, "Hello from a raw write(2) system"..., 38) = 38` lines — proof that our wrapper reached the kernel.

> **Note:** the inline assembly is specific to **x86-64 Linux**.
> The syscall numbers and calling convention are different on other architectures.
