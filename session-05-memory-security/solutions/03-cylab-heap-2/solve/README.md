...
Solution script and a containerized solver environment for the "heap2" challenge.
...
Runs the exploit against the challenge service started via `../deploy/README.md` (listening on `localhost:31012`).
...
docker run --rm -it --network host \
    -v "$(pwd):/solve" \
    heap2-solver \
    python3 /solve/exploit.py REMOTE HOST=127.0.0.1 PORT=31012
```
...
docker run --rm -it \
    -v "$(pwd):/solve" \
    heap2-solver \
    python3 /solve/exploit.py REMOTE HOST=1.2.3.4 PORT=31012
```
