...
Solution script and a containerized solver environment for the "demo-heap-0" challenge.
...
1. Build the solver image:
   ...
   ```console
   docker build -t demo-heap-0-solver .
   ```
...
Runs the exploit against the challenge service started via `../deploy/README.md` (listening on `localhost:31000`).
...
docker run --rm -it --network host \
    -v "$(pwd):/solve" \
    demo-heap-0-solver \
    python3 /solve/exploit.py REMOTE HOST=127.0.0.1 PORT=31000
```
...
docker run --rm -it \
    -v "$(pwd):/solve" \
    demo-heap-0-solver \
    python3 /solve/exploit.py REMOTE HOST=1.2.3.4 PORT=31000
```
