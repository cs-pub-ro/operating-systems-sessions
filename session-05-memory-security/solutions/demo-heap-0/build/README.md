...
Source code and build environment for the "demo-heap-0" challenge.
...
```console
docker build -t demo-heap-0-builder .
docker run --rm -v "$(pwd):/build" demo-heap-0-builder make
```
...
```console
docker run --rm -v "$(pwd):/build" demo-heap-0-builder make clean
```
