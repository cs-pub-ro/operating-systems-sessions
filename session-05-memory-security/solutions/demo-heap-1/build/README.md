...
Source code and build environment for the "demo-heap-1" challenge.
...
```console
docker build -t demo-heap-1-builder .
docker run --rm -v "$(pwd):/build" demo-heap-1-builder make
```
...
```console
docker run --rm -v "$(pwd):/build" demo-heap-1-builder make clean
```
