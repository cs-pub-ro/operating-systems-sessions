...
Deployment configuration for the "heap0" challenge. Uses `xinetd` inside
...
```console
docker build -t heap0-deploy -f Dockerfile ..
```

## Running the container

```console
docker run -d --rm -p 31010:31337 --name heap0-container heap0-deploy
```

The challenge is now reachable at `localhost:31010`:
