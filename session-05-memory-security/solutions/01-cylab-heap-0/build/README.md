...
Source code and build environment for the "heap0" challenge.
...
```console
docker build -t heap0-builder .
docker run --rm -v "$(pwd):/build" heap0-builder make
```
...
```console
docker run --rm -v "$(pwd):/build" heap0-builder make clean
```
