...
Vendor binaries, firmware, a legacy build whose source tree is long gone, a binary you are auditing for a security review — in all of these, "read the disassembly, understand the calling convention, find the one wrong constant, change exactly that" is the entire skill.
It is the same skeleton as every source-level fix in this session — *"which piece of memory does this instruction assume is a certain size, and is it actually that size?"* — just with one extra step of translation, from assembly back to intent.
