...
static void record_grade(struct stats *st, int grade)
{
	...
	/*
	 * FIX: valid grades run from 0 to 100, so grade / 10 runs from 0 to
	 * *10* -- one index too far for an array of NBUCKETS == 10 entries.
	 * The input validation in main() rejects 101 and above, which is why
	 * this looked safe. A perfect score belongs in the top bucket.
	 *
	 * counts[10] is the memory that immediately follows the array inside
	 * struct stats, which happens to be `total`: every grade of 100 was
	 * silently incrementing the wrong field.
	 */
	if (bucket >= NBUCKETS)
		bucket = NBUCKETS - 1;
